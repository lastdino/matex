...
test('draft purchase order cannot be downloaded as PDF', function () {
    ...
    $this->actingAs($user)
        ->get(route('procurement.purchase-orders.pdf', $po))
        ->assertStatus(403);
});
...
test('PDF download button is hidden for draft purchase orders in show page', function () {
    ...
    Livewire::actingAs($user)
        ->test('procflow::procurement.purchase-orders.show', ['po' => $po])
        ->assertDontSee(route('procurement.purchase-orders.pdf', $po));
});
...
test('PDF download button is visible for issued purchase orders in show page', function () {
    ...
    Livewire::actingAs($user)
        ->test('procflow::procurement.purchase-orders.show', ['po' => $po])
        ->assertSee(route('procurement.purchase-orders.pdf', $po));
});
