...
<div class="p-6 space-y-6">
    ...
    <div class="flex flex-wrap items-end gap-4">
        ...
        <div>
            <flux:button variant="primary" wire:click="openCreateMaterial">{{ __('matex::materials.buttons.new') }}</flux:button>
        </div>
    </div>
                        ...
                        <flux:table.cell>
                            <div class="flex justify-end">
                                <flux:dropdown>
                                    ...
                                    <flux:menu>
                                        <flux:menu.item href="{{ route('matex.materials.show', ['material' => $m->id]) }}" icon="clipboard-document-check">{{ __('matex::materials.buttons.view') }}</flux:menu.item>
                                        <flux:menu.item href="{{ route('matex.materials.issue', ['material' => $m->id]) }}" icon="arrow-left-start-on-rectangle">{{ __('matex::materials.buttons.issue') }}</flux:menu.item>
                                        <flux:menu.item href="{{ route('matex.settings.labels') }}" icon="qr-code">{{ __('matex::materials.buttons.shelf_labels') }}</flux:menu.item>
                                        <flux:menu.item icon="qr-code" wire:click="openTokenModal({{ $m->id }})">{{ __('matex::materials.buttons.issue_token') }}</flux:menu.item>
                                        @php($sds = \Illuminate\Support\Facades\Schema::hasTable('media') ? $m->getFirstMedia('sds') : null)
                                        @if($sds)
                                            @php($dl = URL::temporarySignedRoute('matex.materials.sds.download', now()->addMinutes(10), ['material' => $m->id]))
                                            <flux:menu.item icon="document-text" href="{{ $dl }}" target="_blank">{{ __('matex::materials.sds.download') }}</flux:menu.item>
                                        @endif
                                        <flux:menu.item icon="arrow-up-tray" wire:click="openSdsModal({{ $m->id }})">{{ __('matex::materials.sds.open_modal') }}</flux:menu.item>
                                        <flux:menu.item icon="pencil-square" variant="danger" wire:click="openEditMaterial({{ $m->id }})">{{ __('matex::materials.buttons.edit') }}</flux:menu.item>
                                        @if($m->is_active)
    ...
    <flux:modal wire:model="showTokenModal" name="issue-token">
        <div class="w-full md:w-[40rem] max-w-full space-y-4">
            ...
            <div class="space-y-3">
                <flux:input wire:model.defer="tokenForm.token" label="{{ __('matex::materials.token_modal.token') }}" />
                <div class="grid gap-3 md:grid-cols-3">
                    <flux:input wire:model.defer="tokenForm.unit_purchase" label="{{ __('matex::materials.token_modal.unit_purchase') }}" placeholder="e.g. case" />
                    <flux:input type="number" step="0.000001" min="0" wire:model.defer="tokenForm.default_qty" label="{{ __('matex::materials.token_modal.default_qty') }}" />
                    <flux:switch wire:model.defer="tokenForm.enabled" label="{{ __('matex::materials.token_modal.enabled') }}" />
                </div>
                <flux:input type="datetime-local" wire:model.defer="tokenForm.expires_at" label="{{ __('matex::materials.token_modal.expires_at') }}" />
            </div>
            <div class="flex justify-end gap-2">
                <flux:button variant="outline" wire:click="$set('showTokenModal', false)">{{ __('matex::materials.buttons.cancel') }}</flux:button>
    ...
    <flux:modal wire:model.self="showSdsModal" name="sds-form">
        <div class="w-full md:w-[36rem] max-w-full">
            ...
            <div class="space-y-4">
                @if($sdsMaterialId)
                    @php($current = \Illuminate\Support\Facades\Schema::hasTable('media') ? optional(\Lastdino\Matex\Models\Material::find($sdsMaterialId))->getFirstMedia('sds') : null)
                    @if($current)
                ...
                <div>
                    ...
                    <div class="mt-3 flex items-center gap-2">
                        ...
                        <flux:button variant="ghost" @click="$dispatch('close-modal', { name: 'sds-form' })">{{ __('matex::materials.buttons.cancel') }}</flux:button>
                    </div>
