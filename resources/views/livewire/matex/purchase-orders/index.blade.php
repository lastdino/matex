...
new class extends Component
{
    ...
    public function exportExcel(): ?StreamedResponse
    {
        ...
        if ($from === '' || $to === '') {
            $this->addError('receivingDate', __('matex::po.export.validation.receiving_required'));
        ...
        $baseBeforeOptionHeaders = [
            ...
            __('matex::po.export.excel.headers.name'),
            __('matex::po.export.excel.headers.manufacturer'),
        ];
        $baseAfterOptionHeaders = [
            __('matex::po.export.excel.headers.qty_ordered'),
            __('matex::po.export.excel.headers.qty_received'),
            __('matex::po.export.excel.headers.unit_price'),
            __('matex::po.export.excel.headers.amount'),
            __('matex::po.export.excel.headers.note'),
        ];
        ...
        foreach ($items as $ri) {
            ...
            /** @var \Illuminate\Support\Collection<int, \Lastdino\Matex\Models\PurchaseOrderItemOptionValue>|null $ovc */
        ...
        if ($rowGroup !== null) {
            ...
            foreach ($items as $ri0) {
                ...
                $rowKey = __('matex::po.export.excel.matrix.unset');
                $colKey = __('matex::po.export.excel.matrix.unset');
                if ($ovc0) {
                    foreach ($ovc0 as $ov0) {
                        $gid0 = (int) ($ov0->group?->id ?? 0);
                        if ($rowGroup && $gid0 === (int) $rowGroup['id']) {
                            $rowKey = (string) ($ov0->option?->name ?? __('matex::po.export.excel.matrix.unset'));
                        }
                        if ($colGroup && $gid0 === (int) $colGroup['id']) {
                            $colKey = (string) ($ov0->option?->name ?? __('matex::po.export.excel.matrix.unset'));
                        }
                    }
                }
                $rowKeys[$rowKey] = true;
            ...
            if ($colGroup === null) {
                $colKeys = [__('matex::po.export.excel.matrix.unset') => true];
            }
                ...
                if ($ovc1) {
                    foreach ($ovc1 as $ov1) {
                        ...
                        if ($rowGroup && $gid1 === (int) $rowGroup['id']) {
                            $rowKey = (string) ($ov1->option?->name ?? __('matex::po.export.excel.matrix.unset'));
                        }
                        if ($colGroup && $gid1 === (int) $colGroup['id']) {
                            $colKey = (string) ($ov1->option?->name ?? __('matex::po.export.excel.matrix.unset'));
                        }
                    }
                }
            ...
            // Final totals row
            $totalsRow = [__('matex::po.export.excel.matrix.total')];
            foreach ($colValues as $ck) {
            ...
            $matrixSheet->setTitle(__('matex::po.export.excel.matrix.sheet_title'));
    ...
    {
        return \Lastdino\Matex\Models\Material::query()
            ->active()
            ->orderBy('sku')
            ->get();
    }
    ...
    public function getPoSupplierPreviewProperty(): array
    {
        ...
        foreach (array_values($this->poForm['items']) as $idx => $line) {
            ...
            /** @var \Lastdino\Matex\Models\Material|null $mat */
            $mat = \Lastdino\Matex\Models\Material::find((int) $materialId);
            if (! $mat || is_null($mat->preferred_supplier_id)) {
                continue;
            }
            $sid = (int) $mat->preferred_supplier_id;
            if (! isset($groups[$sid])) {
                /** @var \Lastdino\Matex\Models\Supplier|null $sup */
                $sup = \Lastdino\Matex\Models\Supplier::query()->find($sid);
                $groups[$sid] = [
                    'supplier_id' => $sid,
    ...
    public function savePoFromModal(): void
    {
        ...
        $rules = (new \Lastdino\Matex\Http\Requests\StorePurchaseOrderRequest)->rules();
        ...
        foreach ($items as $idx => $line) {
            ...
            /** @var \Lastdino\Matex\Models\Material|null $mat */
            $mat = \Lastdino\Matex\Models\Material::find((int) $matId);
            if (! $mat || is_null($mat->preferred_supplier_id)) {
        ...
        $optionService = app(\Lastdino\Matex\Services\OptionSelectionService::class);
        $factory = app(\Lastdino\Matex\Services\PurchaseOrderFactory::class);
        $approval = app(\Lastdino\Matex\Services\ApprovalFlowRegistrar::class);
    ...
    public function updatedPoFormSupplierId($value): void
    {
        ...
        /** @var \Lastdino\Matex\Models\Supplier|null $sup */
        $sup = \Lastdino\Matex\Models\Supplier::query()->find($supplierId);
        if ($sup) {
     ...
     */
    protected function resolveMaterialTaxRate(?\Lastdino\Matex\Models\Material $material, array $taxSet): float
    {
    ...
    public function onMaterialChanged(int $index, $materialId): void
    {
        ...
        /** @var \Lastdino\Matex\Models\Material|null $material */
        $material = \Lastdino\Matex\Models\Material::find($materialId);
        if ($material) {
        ...
        foreach ($this->poForm['items'] as $i => $line) {
            ...
            if (! $locked) {
                ...
                if (! is_null($materialId)) {
                    /** @var \Lastdino\Matex\Models\Material|null $material */
                    $material = \Lastdino\Matex\Models\Material::find((int) $materialId);
    ...
    {
        try {
            $flowIdStr = \Lastdino\Matex\Models\AppSetting::get('approval_flow.purchase_order_flow_id');
     ...
     * 共通: よく使うサービスの取得。
     *
     * @return array{0:\Lastdino\Matex\Services\OptionSelectionService,1:\Lastdino\Matex\Services\PurchaseOrderFactory,2:\Lastdino\Matex\Services\ApprovalFlowRegistrar}
     */
     ...
     */
    protected function redirectToPoShow(\Lastdino\Matex\Models\PurchaseOrder $po): void
    {
        if (\Illuminate\Support\Facades\Route::has('matex.purchase-orders.show')) {
...
<div class="p-6 space-y-6">
    ...
    <div class="flex flex-wrap items-end gap-3">
        ...
        <div class="w-full sm:w-48">
            <flux:select wire:model.live="requesterId">
                <option value="">{{ __('matex::po.filters.requester') }}</option>
        ...
        <flux:field>
            <flux:label>{{ __('matex::po.filters.issue_from_to') }}</flux:label>
            <div class="flex items-center gap-2">
                <flux:input type="date" wire:model.live="issueDate.start" placeholder="{{ __('matex::po.filters.issue_from') }}" />
                <flux:input type="date" wire:model.live="issueDate.end" placeholder="{{ __('matex::po.filters.issue_to') }}" />
            </div>
        </flux:field>
        <flux:field>
            <flux:label>{{ __('matex::po.filters.expected_from_to') }}</flux:label>
            <div class="flex items-center gap-2">
                <flux:input type="date" wire:model.live="expectedDate.start" placeholder="{{ __('matex::po.filters.expected_from') }}" />
                <flux:input type="date" wire:model.live="expectedDate.end" placeholder="{{ __('matex::po.filters.expected_to') }}" />
            </div>
        </flux:field>

        <div class="w-full sm:flex-1 min-w-64">
            <flux:input wire:model.live.debounce.300ms="q" placeholder="{{ __('matex::po.filters.search_placeholder') }}" />
        </div>
        <div class="w-full sm:w-40">
            <flux:select wire:model.live="status">
                <option value="">{{ __('matex::po.filters.status_all') }}</option>
                <option value="draft">{{ __('matex::po.status.draft') }}</option>
                <option value="issued">{{ __('matex::po.status.issued') }}</option>
                <option value="receiving">{{ __('matex::po.status.receiving') }}</option>
                <option value="closed">{{ __('matex::po.status.closed') }}</option>
                <option value="canceled">{{ __('matex::po.status.canceled') }}</option>
            </flux:select>
        </div>
        <div class="ms-auto flex gap-2 w-full sm:w-auto">
            <flux:button variant="outline" wire:click="clearFilters">{{ __('matex::po.buttons.clear_filters') }}</flux:button>
            <flux:button variant="primary" wire:click="openCreatePo">{{ __('matex::po.buttons.new_po') }}</flux:button>
            <flux:button variant="outline" wire:click="openAdhocPo">{{ __('matex::po.buttons.adhoc_order') }}</flux:button>
            <flux:button variant="outline" wire:click="openExportModal">{{ __('matex::po.export.history_button') }}</flux:button>
        </div>
    </div>
            ...
            <flux:table.rows>
                @forelse($this->orders as $po)
                    <flux:table.row>
                        <flux:table.cell>
                            <flux:link
                               href="{{ route('matex.purchase-orders.show', ['po' => $po->id]) }}">
                        ...
                        <flux:table.cell>{{ \Lastdino\Matex\Support\Format::moneyTotal($po->total) }}</flux:table.cell>
                    </flux:table.row>
                @empty
                    <flux:table.row>
    ...
    <flux:modal wire:model.self="showExportModal" name="export-history" class="w-full md:w-[40rem] max-w-full">
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                <flux:field>
                    <flux:label>{{ __('matex::po.export.fields.row_group') }}</flux:label>
                    <flux:select wire:model.live="rowGroupId">
                        <option value="">{{ __('matex::po.export.fields.row_auto') }}</option>
                ...
                <flux:field>
                    <flux:label>{{ __('matex::po.export.fields.col_group') }}</flux:label>
                    <flux:select wire:model.live="colGroupId">
                        <option value="">{{ __('matex::po.export.fields.col_none') }}</option>
                        @foreach($this->optionGroups as $grp)
                            <option value="{{ $grp->id }}">{{ $grp->name }}</option>
                        @endforeach
                    </flux:select>
        ...
        <div class="flex items-center justify-end gap-2 mt-4">
            <flux:button variant="ghost" wire:click="$set('showExportModal', false)">{{ __('matex::po.buttons.cancel') }}</flux:button>
            <flux:button variant="primary" wire:click="exportExcel" wire:target="exportExcel" wire:loading.attr="disabled">
                <span wire:loading.remove wire:target="exportExcel">{{ __('matex::po.export.buttons.download') }}</span>
                <span wire:loading wire:target="exportExcel">{{ __('matex::po.export.buttons.creating') }}</span>
            </flux:button>
        </div>
    </flux:modal>
    ...
    <flux:modal wire:model.self="showPoModal" name="create-po" class="w-full md:w-[64rem] max-w-full" :dismissible="false">
        <div class="w-full md:w-[64rem] max-w-full">
            <h3 class="text-lg font-semibold mb-3">{{ __('matex::po.create.title') }}</h3>
            ...
            <flux:textarea
                ...
                placeholder="{{ __('matex::po.create.delivery.placeholder') }}"
                    ...
                    <table class="min-w-full text-sm">
                        <thead>
                            <tr class="text-left text-neutral-500">
                                ...
                                <th class="py-2 px-3">{{ __('matex::po.create.table.unit_price') }}</th>
                            ...
                            @foreach($poForm['items'] as $i => $row)
                                <tr class="border-t">
                                    ...
                                    <td class="py-2 px-3">
                                        <flux:textarea
                                            rows="2"
                                            placeholder="{{ __('matex::po.common.note') }}"
                                    ...
                                    <td class="py-2 px-3 w-28">
                                        <div class="w-28">
                                            <flux:input wire:model="poForm.items.{{ $i }}.unit_purchase" placeholder="{{ __('matex::po.common.unit_example') }}"/>
                                        </div>
                                    </td>
                                    <td class="py-2 px-3">
                                        <div class="w-28">
            ...
            <div class="mt-4">
                <div class="text-sm text-neutral-600 mb-2">{{ __('matex::po.create_preview.title') }}</div>
                ...
                @if (!empty($preview))
                    <div class="rounded border border-amber-300 bg-amber-50 dark:bg-amber-900/20 p-3 text-amber-800 dark:text-amber-200">
                        <div class="font-medium mb-2">{{ __('matex::po.create_preview.split_notice', ['count' => count($preview)]) }}</div>
                            ...
                            @foreach ($preview as $p)
                                <li class="flex items-center justify-between rounded border border-amber-200 dark:border-amber-800 bg-white dark:bg-neutral-900 px-3 py-2">
                                    <div>
                                        ...
                                        <div class="text-neutral-500">{{ __('matex::po.create_preview.lines') }}: {{ $p['lines'] }}</div>
                                    </div>
                                    <div class="text-right">
                                        <div class="text-neutral-500">{{ __('matex::po.create_preview.subtotal_excl_tax') }}</div>
                                        <div class="font-semibold">{{ \Lastdino\Matex\Support\Format::moneySubtotal($p['subtotal']) }}</div>
                                    </div>
                                </li>
                            @endforeach
                        </ul>
                ...
                @else
                    <div class="text-sm text-neutral-500">{{ __('matex::po.create_preview.select_materials_hint') }}</div>
                @endif

                <div class="text-xs text-neutral-500 mt-2">{{ __('matex::po.create_preview.no_adhoc_hint') }}</div>
            </div>

            <div class="mt-4 flex items-center justify-end gap-3">
                <flux:button variant="outline" x-on:click="$flux.modal('create-po').close()">{{ __('matex::po.buttons.cancel') }}</flux:button>
                <flux:button variant="primary" wire:click="savePoFromModal" wire:loading.attr="disabled">
                    <span wire:loading.remove>{{ __('matex::po.buttons.create') }}</span>
                    <span wire:loading>{{ __('matex::po.buttons.creating') }}</span>
                </flux:button>
            </div>
        </div>
    </flux:modal>
    ...
    <flux:modal wire:model.self="showAdhocPoModal" name="create-adhoc-po" class="w-full md:w-[64rem] max-w-full" :dismissible="false">
        <div class="w-full md:w-[64rem] max-w-full">
            <h3 class="text-lg font-semibold mb-3">{{ __('matex::po.adhoc.title') }}</h3>
            ...
            <flux:textarea
                label="{{ __('matex::po.adhoc.delivery.label') }}"
                placeholder="{{ __('matex::po.adhoc.delivery.placeholder') }}"
                    ...
                    <table class="min-w-full text-sm">
                        <thead>
                            <tr class="text-left text-neutral-500">
                                ...
                                <th class="py-2 px-3">{{ __('matex::po.adhoc.table.note') }}</th>
                                <th class="py-2 px-3">{{ __('matex::po.adhoc.table.unit') }}</th>
                                <th class="py-2 px-3">{{ __('matex::po.adhoc.table.qty') }}</th>
                                <th class="py-2 px-3">{{ __('matex::po.adhoc.table.unit_price') }}</th>
                            ...
                            @foreach($adhocForm['items'] as $i => $row)
                                <tr class="border-t">
                                    ...
                                    <td class="py-2 px-3">
                                        <div class="w-40">
                                            <flux:input
                                                placeholder="{{ __('matex::po.adhoc.placeholders.manufacturer_hint') }}"
                                    ...
                                    <td class="py-2 px-3 w-28">
                                        <div class="w-28">
                                            <flux:input wire:model.live="adhocForm.items.{{ $i }}.unit_purchase" placeholder="{{ __('matex::po.common.unit_example') }}"/>
                                        </div>
                                    </td>
                                    <td class="py-2 px-3">
                                        <div class="w-28">
            ...
            <div class="mt-4 flex items-center justify-end gap-3">
                <flux:button variant="outline" x-on:click="$flux.modal('create-adhoc-po').close()">{{ __('matex::po.buttons.cancel') }}</flux:button>
                <flux:button variant="primary" wire:click="saveAdhocPoFromModal" wire:loading.attr="disabled">
                    <span wire:loading.remove>{{ __('matex::po.buttons.save') }}</span>
                    <span wire:loading>{{ __('matex::po.buttons.saving') }}</span>
                </flux:button>
            </div>
        </div>
    </flux:modal>
