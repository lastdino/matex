...
new class extends Component
{
    ...
    public function save(): void
    {
        ...
        $this->dispatch('notify', text: __('matex::settings.approval.flash.saved'));
    }
...
<div class="p-6 space-y-6">
    ...
    <div class="mx-auto w-full max-w-2xl">
        <flux:heading size="xl" class="mb-6">{{ __('matex::settings.approval.title') }}</flux:heading>

        <div class="space-y-5">
            <flux:select wire:model="purchaseOrderFlowId" placeholder="{{ __('matex::settings.approval.select.placeholder') }}" label="{{ __('matex::settings.approval.select.label') }}">
                @foreach($this->flows as $flow)
                    <flux:select.option :value="$flow->id">{{ $flow->name }} (ID: {{ $flow->id }})</flux:select.option>
                @endforeach
            </flux:select>
            <div class="flex gap-3">
                <flux:button wire:click="save" variant="primary">{{ __('matex::settings.approval.buttons.save') }}</flux:button>
            </div>
        </div>
    </div>
</div>
