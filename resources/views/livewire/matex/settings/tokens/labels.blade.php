...
new class extends Component
{
    ...
    public function makeQrData(string $token): string
    {
        if ($this->payload === 'url') {
            ...
            $url = route('matex.ordering.scan', ['token' => $token], true);
...
<div class="p-6 space-y-6">
    ...
    <div class="flex items-center justify-between print:hidden">
        <h1 class="text-xl font-semibold">{{ __('matex::settings.labels.title') }}</h1>
        <a href="{{ route('matex.settings.tokens') }}" class="text-blue-600 hover:underline">{{ __('matex::settings.labels.to_tokens') }}</a>
    </div>

    <div class="grid gap-4 md:grid-cols-5 items-end print:hidden">
        <flux:field class="md:col-span-2">
            <flux:input wire:model.live.debounce.300ms="search" placeholder="{{ __('matex::settings.labels.filters.search_placeholder') }}" />
        </flux:field>

        <flux:field>
            <flux:select wire:model.live="materialId">
                <option value="">{{ __('matex::settings.labels.filters.all_materials') }}</option>
        ...
        <flux:field label="{{ __('matex::settings.labels.filters.payload') }}">
            <flux:select wire:model.live="payload">
                <option value="token">{{ __('matex::settings.labels.filters.payload_token_only') }}</option>
                <option value="url">{{ __('matex::settings.labels.filters.payload_url') }}</option>
            </flux:select>
        </flux:field>

        <flux:field label="{{ __('matex::settings.labels.filters.per_page') }}">
            ...
            <div class="p-4 border rounded break-inside-avoid">
                <div class="flex items-start justify-between gap-3">
                    <div>
                        ...
                        <div class="text-xs mt-1">{{ __('matex::settings.labels.card.unit') }}: {{ $row->unit_purchase ?? '-' }}</div>
                        @if($row->material)
                            <div class="text-xs text-gray-600">{{ __('matex::settings.labels.card.moq_and_pack', ['moq' => $row->material->moq ?? '-', 'pack' => $row->material->pack_size ?? '-']) }}</div>
                        @endif
